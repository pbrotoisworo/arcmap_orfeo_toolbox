...
with open(log_file, 'w') as f:
    ...
    f.write('\nMinimum Number of Samples in Each Node: {}'.format(input_min_samples_node))
    f.write('\nTermination Criteria for Each Regression Tree: {}'.format(input_termination_criteria))
    f.write('\nCluster Possible Values of a Categorical Variable into K: {}'.format(input_cluster_possible_values_k))
    f.write('\nK-fold Cross Validations: {}'.format(input_k_fold_validations))
    f.write('\nSet Use1seRule flag to false: {}'.format(input_set_use1serule))
    f.write('\nSet TruncatePrunedTree flag to false: {}'.format(input_set_truncate_pruned_tree_to_false))
