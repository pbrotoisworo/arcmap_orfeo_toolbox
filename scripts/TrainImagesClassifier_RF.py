...
with open(log_file, 'w') as f:
    ...
    f.write('\nMaximum Depth of the Tree: {}'.format(input_maximum_depth_tree))
    f.write('\nMinimum Number of Samples in Each Node: {}'.format(input_minimum_number_each_node))
    f.write('\nTermination Criteria for Regression Tree: {}'.format(input_termination_criteria))
    f.write('\nCluster Possible Values of a Categorical Variable into K <= Cat Clusters to Find a Suboptimal Split: {}'.format(input_cluster_categorical_value))
    f.write('\nSize of the Randomly Selected Subset of Features at Each Tree Node: {}'.format(input_size_randomly_selected_features_node))
    f.write('\nMaximum Number of Trees in the Forest: {}'.format(input_maximum_number_trees))
    f.write('\nSufficient Accuracy (OOB error): {}'.format(input_oob_error))
    f.write('\nUser Seed: {}'.format(input_user_seed))
    f.write('\nOutput Confusion Matrix or Contingency Table: {}'.format(out_conf_matrix))
