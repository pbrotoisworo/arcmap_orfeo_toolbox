...
log_file = os.path.join(log_folder, 'TRAINCLASSBOOST' + ts_file_append + '.log')
...
input_train_method = 'boost'
input_boost_type = arcpy.GetParameterAsText(9)
...
# Generate command for boost type
otb_input_boost_type = generate_command('-classifier.boost.t ', False, input_boost_type, False)
command_list.append(otb_input_boost_type)

# Generate command for weak count
otb_input_weak_count = generate_command('-classifier.boost.w ', False, input_weak_count, False)
command_list.append(otb_input_weak_count)

# Generate command for weight trim rate
otb_input_weight_trim_rate = generate_command('classifier.boost.r ', False, input_weight_trim_rate, False)
command_list.append(otb_input_weight_trim_rate)

# Generate command for max depth tree
otb_input_max_depth_tree = generate_command('classifier.boost.m ', False, input_max_depth_tree, False)
...
with open(log_file, 'w') as f:
    ...
    f.write('\nTrain/Validation Sample Ratio: {}'.format(input_train_validation_ratio))
    f.write('\nBound sample number minimum: {}'.format(input_bound_sample_num_min))
    f.write('\nDefault Elevation: {}'.format(input_default_elev))
    f.write('\nBoost Type: {}'.format(input_boost_type))
    f.write('\nWeak Count: {}'.format(input_weak_count))
    f.write('\nWeight Trim Rate: {}'.format(input_weight_trim_rate))
    f.write('\nMaximum Depth of Tree: {}'.format(input_max_depth_tree))
    f.write('\nOutput Confusion Matrix: {}'.format(out_conf_matrix))
    f.write('\nOutput Model: {}'.format(out_model))
